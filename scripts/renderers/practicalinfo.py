...
def render_practical_info(practical_info, course):
    ...
    html.append('<div class="course-practical-page">')
    ...
    venue = practical_info.get("venue", {})
    ...
    if any(venue.get(field) for field in [
        "name",
        "address",
        "room",
        "instructions",
        ...
        html.append('<section class="course-practical-section">')
        html.append(
            section_heading("bi-geo-alt-fill", "Venue")
            )
        ...
        html.append('<div class="course-practical-venue">')
        ...
        if venue.get("address"):
            venue_details.append(
                ...
                f'<div class="course-practical-address">'
        ...
        if venue_details:
            html.append(
                '<div class="course-practical-venue-details">'
                + "\n".join(venue_details)
                + '</div>'
            )

        if venue.get("map_url"):
            map_embed_url = osm_embed_url(venue["map_url"])

            html.append(
                '<div class="course-practical-map-column">'
                '<div class="course-practical-map">'
                f'<iframe '
                f'src="{map_embed_url}" '
                'title="Venue map" '
                'loading="lazy">'
                '</iframe>'
                '</div>'
                f'<a class="course-practical-map-link" '
    ...
    transport = practical_info.get("transport", {})

    if transport.get("description") or transport.get("public_transport") \
            or transport.get("parking") or transport.get("links"):

        html.append('<section class="course-practical-section">')
        html.append(
            section_heading("bi-bus-front-fill", "Transport")
        )
        ...
        if links:
            html.append('<ul class="course-practical-links">')
    ...
    accommodation = practical_info.get("accommodation", {})
    ...
    if accommodation.get("description") or hotels:
        ...
        html.append('<section class="course-practical-section">')
        html.append(
            section_heading("bi-building", "Accommodation")
        )
        ...
        if hotels:
            html.append(
                '<div class="course-practical-table-wrapper">'
            )
            html.append('<table class="course-practical-table">')
            html.append(
                '<thead>'
                '<tr>'
                '<th>Hotel</th>'
    ...
    food = practical_info.get("food", {})

    if food.get("description") or food.get("dietary_information"):

        html.append('<section class="course-practical-section">')
        html.append(
            section_heading("bi-cup-hot-fill", "Food &amp; refreshments")
        )
    ...
    additional = [
        item
        for item in practical_info.get("additional", [])
    ...
    if additional:
        ...
        html.append('<section class="course-practical-section">')
        html.append(
            section_heading("bi-file-earmark-text-fill", "Other information")
        )

        html.append('<div class="course-practical-additional">')

        for item in additional:
            html.append(
                '<div class="course-practical-additional-item">'
                f'<h3>{item["title"]}</h3>'
                f'<p>{item["content"]}</p>'
                '</div>'
            )
